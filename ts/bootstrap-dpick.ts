interface iOptions {
    inputDateStartName: string;
    inputDateEndName: string;
}
export default class BootstrapDPick {
    private currentDate: Date;
    private container: HTMLElement;
    private startDayElement: HTMLElement | null = null;
    private endDayElement: HTMLElement | null = null;
    private startDate: any = null; // Store as ISO string
    private endDate: any = null; // Store as ISO string
    private startDateChangeEvent: CustomEvent<{}>;
    private endDateChangeEvent: CustomEvent<{}>;
    private options: iOptions = {
        inputDateStartName: "inputDateStartName",
        inputDateEndName: "inputDateEndName",
    };

    constructor(containerClassName: string, options: iOptions) {
    ...
    private renderBody(): void {
        ...
        if (this.startDate) {
            ...
            if (startDayCell) {
                ...
                this.startDayElement = startDayCell as HTMLElement;
            }
        }
        ...
        if (this.endDate) {
            ...
            if (endDayCell) {
                ...
                this.endDayElement = endDayCell as HTMLElement;
            }
        }
    }
    ...
    private renderFooter(): void {
        ...
        if (this.startDate) {
            const startInput: any = this.container.querySelector('.dpick-footer-actions-start .dpick-footer-action-input');
            const startDateObj = new Date(this.startDate);
            const startActionDateElement = this.container.querySelector('.dpick-footer-actions-start .dpick-footer-actions-date');
            if (startActionDateElement) startActionDateElement.textContent = startDateObj.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' });
            if (startInput) startInput.value = this.startDate;
        }

        if (this.endDate) {
            const endInput: any = this.container.querySelector('.dpick-footer-actions-end .dpick-footer-action-input');
            const endDateObj = new Date(this.endDate);
            const endActionDateElement = this.container.querySelector('.dpick-footer-actions-end .dpick-footer-actions-date');
            if (endActionDateElement) endActionDateElement.textContent = endDateObj.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' });
    ...
    private resetSelections(): void {
        ...
        const startActionDateElement = this.container.querySelector('.dpick-footer-actions-start .dpick-footer-actions-date');
        const endActionDateElement = this.container.querySelector('.dpick-footer-actions-end .dpick-footer-actions-date');
        if (startActionDateElement) startActionDateElement.textContent = '';
        if (endActionDateElement) endActionDateElement.textContent = '';
    ...
    private actionBody(): void {
        ...
        const resetSelections = () => {
            if (this.startDayElement) this.startDayElement.classList.remove('dpick-day-start');
            if (this.endDayElement) this.endDayElement.classList.remove('dpick-day-end');
            this.startDayElement = null;
            this.endDayElement = null;
            if (startActionDateElement) startActionDateElement.textContent = '';
            if (endActionDateElement) endActionDateElement.textContent = '';
            if (startInput) startInput.value = ''; // Reset the input values
            if (endInput) endInput.value = '';
        };

        // Method to handle day clicks
        const handleDayClick = (dayElement: any) => {
            ...
            if (!dayElement.classList.contains('opacity-50') && dayElement !== this.startDayElement && dayElement !== this.endDayElement) {
                if (!this.startDayElement) {
                    ...
                    if (startActionDateElement) startActionDateElement.textContent = formattedDate; // Display the date
                    if (startInput) startInput.value = dayElement.dataset.date; // Set input value
                    ...
                    if (this.startDayElement && new Date(this.startDayElement.dataset.date as any) >= selectedDate) {
                    ...
                    if (endActionDateElement) endActionDateElement.textContent = formattedDate; // Display the date
                    if (endInput) endInput.value = dayElement.dataset.date; // Set input value
        ...
        this.container.querySelectorAll('.dpick-day:not(.opacity-50)').forEach(day => {
            day.addEventListener('click', () => handleDayClick(day as HTMLElement));
        });
    }

    public onStartDateChange(listener: (event: CustomEvent) => void): void {
        this.container.addEventListener('startDateChange', listener as EventListener);
    }

    public onEndDateChange(listener: (event: CustomEvent) => void): void {
        this.container.addEventListener('endDateChange', listener as EventListener);
    }
}